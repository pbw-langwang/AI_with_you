...
export function runDiagnosis(appState: AppState): DiagnosisResult {
  appState.ui.diagnosis = {
    active: true,
    lines: ["详细描述一下您的症状，我在听！"],
  };
  if (appState.ui.routeGuide) {
    appState.ui.routeGuide.visible = false;
  }
  const speakParts = ["好的", appState.ui.diagnosis.lines[0]];
  ...
  return {
    speakText: speakParts.join("。"),
    suffixText: "您可以直接输入我要去某某门诊，我会给您院内导航！",
  };
}
...
export function runMallFun(appState: AppState): DiagnosisResult {
  appState.ui.diagnosis = {
    active: true,
    lines: [
  ...
  const speakParts = ["好的", ...appState.ui.diagnosis.lines.slice(0, 2)];
  for (let i = 0; i < speakParts.length; i++) {
    const isFirst = i === 0;
    const isLast = i === speakParts.length - 1;
    appState.avatar.instance.speak(
  ...
  return {
    speakText: speakParts.join("。"),
    suffixText: "您可以直接输入我要去某某商家，我会给您商场内导航！",
  };
}
...
export function runMallFood(appState: AppState): DiagnosisResult {
  appState.ui.diagnosis = {
    active: true,
    lines: [
  ...
  return {
    speakText: speakParts.join("。"),
    suffixText: "您可以直接输入我要去某某餐厅，我会给您商场内导航！",
  };
}
